@AbapCatalog.sqlViewName: 'ZCDS_43_LOG'
@AbapCatalog.compiler.compareFilter: true
@AbapCatalog.preserveKey: true
@AccessControl.authorizationCheck: #CHECK
@EndUserText.label: 'CDS - Analytics - CUBE'
@Analytics.dataCategory: #CUBE
define view zcds_43_cube_flights_logali

  as select from sbook

  association [0..1] to I_CalendarDate              as _CalendarDate on  $projection.FlightDate = _CalendarDate.CalendarDate
  association [0..1] to zcds_39_dim_airlines_logali as _Airlines     on  $projection.Airline = _Airlines.Airline
  association [0..1] to zcds_40_dim_conn_logali     as _Connection   on  $projection.Airline          = _Connection.Airline
                                                                     and $projection.FlightConnection = _Connection.FlightConnection
  association [0..1] to zcds_41_dim_cust_logali     as _Customer     on  $projection.Customer = _Customer.Customer
  association [0..1] to zcds_42_dim_agency_logali   as _TravelAgency on  $projection.TravelAgency = _TravelAgency.TravelAgency
